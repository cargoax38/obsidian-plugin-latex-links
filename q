...
DDEESSCCRRIIPPTTIIOONN
       ccuurrll  is  a  tool  for  transferring  data from or to a server. It supports these protocols: DICT, FILE, FTP, FTPS, GOPHER, GOPHERS, HTTP, HTTPS, IMAP, IMAPS, LDAP, LDAPS, MQTT, POP3, POP3S, RTMP,
...
OOUUTTPPUUTT
       If  not  told  otherwise, curl writes the received data to stdout. It can be instructed to instead save that data into a local file, using the _-_o_, _-_-_o_u_t_p_u_t or _-_O_, _-_-_r_e_m_o_t_e_-_n_a_m_e options. If curl is
...
PPRROOGGRREESSSS MMEETTEERR
       ...
       curl  displays  this  data  to the terminal by default, so if you invoke curl to do an operation and it is about to write data to the terminal, it _d_i_s_a_b_l_e_s the progress meter as otherwise it would
       mess up the output mixing progress meter and response data.

       If you want a progress meter for HTTP POST or PUT requests, you need to redirect the response output to a file, using shell redirect (>), _-_o_, _-_-_o_u_t_p_u_t or similar.

       This does not apply to FTP upload as that operation does not spit out any response data to the terminal.

       If you prefer a progress "bar" instead of the regular meter, _-_#_, _-_-_p_r_o_g_r_e_s_s_-_b_a_r is your friend. You can also disable the progress meter completely with the _-_s_, _-_-_s_i_l_e_n_t option.
...
OOPPTTIIOONNSS
       ...
       _-_d_, _-_-_d_a_t_a for example, requires a space between it and its value.
       ...
       In  general,  all boolean options are enabled with --ooppttiioonn and yet again disabled with --nnoo--option. That is, you use the same option name but prefix it with "no-". However, in this list we mostly
       only list and show the --option version of them.

       --abstract-unix-socket <path>
              (HTTP) Connect through an abstract Unix domain socket, instead of using the network.  Note: netstat shows the path of an abstract socket prefixed  with  '@',  however  the  <path>  argument
       ...
       --anyauth
              ...
              sponse-headers,  thus  possibly  inducing an extra network round-trip. This is used instead of setting a specific authentication method, which you can do with _-_-_b_a_s_i_c, _-_-_d_i_g_e_s_t, _-_-_n_t_l_m, and
              _-_-_n_e_g_o_t_i_a_t_e.

              Using _-_-_a_n_y_a_u_t_h is not recommended if you do uploads from stdin, since it may require data to be sent twice and then the client must be able to rewind. If the need should arise when upload‐
              ing from stdin, the upload operation will fail.
       ...
       --basic
              ...
              sets a different authentication method (such as _-_-_n_t_l_m, _-_-_d_i_g_e_s_t, or _-_-_n_e_g_o_t_i_a_t_e).
       ...
       --cacert <file>
              ...
              (iOS  and  macOS only) If curl is built against Secure Transport, then this option is supported for backward compatibility with other SSL engines, but it should not be set. If the option is
              not set, then curl will use the certificates in the system and user Keychain to verify the peer, which is the preferred method of verifying the peer's certificate chain.

              (Schannel only) This option is supported for Schannel in Windows 7 or later with libcurl 7.60 or later. This option is supported for backward compatibility with other SSL  engines;  instead
       ...
       --capath <dir>
              ...
              be in PEM format, and if curl is built against OpenSSL, the directory must have been processed using the c_rehash utility supplied with OpenSSL. Using  _-_-_c_a_p_a_t_h  can  allow  OpenSSL-powered
              curl to make SSL-connections much more efficiently than using _-_-_c_a_c_e_r_t if the --cacert file contains many CA certificates.
       ...
       -E, --cert <certificate[:password]>
              ...
              Transport,  or  PEM  format  if using any other engine. If the optional password is not specified, it will be queried for on the terminal. Note that this option assumes a "certificate" file
              that is the private key and the client certificate concatenated! See _-_E_, _-_-_c_e_r_t and _-_-_k_e_y to specify them independently.
              ...
              ning with "pkcs11:" will be interpreted as a PKCS#11 URI. If a PKCS#11 URI is provided, then the _-_-_e_n_g_i_n_e option will be set as "pkcs11" if none was provided and the _-_-_c_e_r_t_-_t_y_p_e option will
              be set as "ENG" if none was provided.

              (iOS  and  macOS only) If curl is built against Secure Transport, then the certificate string can either be the name of a certificate/private key in the system or user keychain, or the path
              to a PKCS#12-encoded certificate and private key. If you want to use a file from the current directory, please precede it with "./" prefix, in order to avoid confusion with a nickname.

              (Schannel only) Client certificates must be specified by a path expression to a certificate store. (Loading PFX is not supported; you can import it to a store first). You  can  use  "<store
              location>\<store  name>\<thumbprint>"  to refer to a certificate in the system certificates store, for example, "CurrentUser\MY\934a7ac6f8a5d579285a74fa61e19f23ddfe8d7a". Thumbprint is usu‐
              ally a SHA-1 hex string which you can see in certificate details. Following store locations are supported: CurrentUser, LocalMachine, CurrentService, Services,  CurrentUserGroupPolicy,  Lo‐
       ...
       -K, --config <file>
              ...
              without the initial double dashes and if so, the colon or equals characters can be used as separators. If the option is specified with one or two dashes, there can be  no  colon  or  equals
              ...
              Note that to be able to specify a URL in the config file, you need to specify it using the _-_-_u_r_l option, and not by simply writing the URL on its own line. So,  it  could  look  similar  to
              this:
              ...
              When  curl is invoked, it (unless _-_q_, _-_-_d_i_s_a_b_l_e is used) checks for a default config file and uses it if found, even when _-_K_, _-_-_c_o_n_f_i_g is used. The default config file is checked for in the
       ...
       --connect-to <HOST1:PORT1:HOST2:PORT2>
              ...
              For  a  request to the given HOST1:PORT1 pair, connect to HOST2:PORT2 instead.  This option is suitable to direct requests at a specific server, e.g. at a specific cluster node in a cluster
              of servers. This option is only used to establish the network connection. It does NOT affect the hostname/port that is used for TLS/SSL (e.g. SNI, certificate verification) or for  the  ap‐
              plication protocols. "HOST1" and "PORT1" may be the empty string, meaning "any host/port". "HOST2" and "PORT2" may also be the empty string, meaning "use the request's original host/port".

              A  "host"  specified  to this option is compared as a string, so it needs to match the name used in request URL. It can be either numerical such as "127.0.0.1" or the full host name such as
              "example.org".
       ...
       -c, --cookie-jar <filename>
              ...
              This command line option will activate the cookie engine that makes curl record and use cookies. Another way to activate it is to use the _-_b_, _-_-_c_o_o_k_i_e option.

              If  the cookie jar cannot be created or written to, the whole curl operation will not fail or even report an error clearly. Using _-_v_, _-_-_v_e_r_b_o_s_e will get a warning displayed, but that is the
       ...
       -b, --cookie <data|filename>
              ...
              incoming cookies, which may be handy if you are using this in combination with the _-_L_, _-_-_l_o_c_a_t_i_o_n option or do multiple URL transfers on the same invoke. If the file name is exactly a minus
              ...
              The file specified with _-_b_, _-_-_c_o_o_k_i_e is only used as input. No cookies will be written to the file. To store cookies, use the _-_c_, _-_-_c_o_o_k_i_e_-_j_a_r option.
              ...
              Users often want to both read cookies from a file and write updated cookies back to a file, so using both _-_b_, _-_-_c_o_o_k_i_e and _-_c_, _-_-_c_o_o_k_i_e_-_j_a_r in the same command line is common.
       ...
       --create-dirs
              When used in conjunction with the _-_o_, _-_-_o_u_t_p_u_t option, curl will create the necessary local directory hierarchy as needed. This option creates the directories mentioned with the _-_o_,  _-_-_o_u_t_‐
              _p_u_t option, nothing else. If the --output file name uses no directory, or if the directories it mentions already exist, no directories will be created.
       ...
       --curves <algorithm list>
              ...
              _-_-_c_u_r_v_e_s allows a OpenSSL powered curl to make SSL-connections with exactly the (EC) curve requested by the client, avoiding nontransparent client/server negotiations.
       ...
       --data-binary <data>
              ...
              If  you  start  the data with the letter @, the rest should be a filename. Data is posted in a similar manner as _-_d_, _-_-_d_a_t_a does, except that newlines and carriage returns are preserved and
              conversions are never done.

              Like _-_d_, _-_-_d_a_t_a the default content-type sent to the server is application/x-www-form-urlencoded. If you want the data to be treated as arbitrary binary data by the server then set the con‐
       ...
       --data-urlencode <data>
              ...
              To be CGI-compliant, the <data> part should begin with a _n_a_m_e followed by a separator and a content specification. The <data> part can be passed to curl using one of the following syntaxes:

              content
                     This  will  make curl URL-encode the content and pass that on. Just be careful so that the content does not contain any = or @ symbols, as that will then make the syntax match one of
              ...
              name@filename
                     ...
                     _n_a_m_e_=_u_r_l_e_n_c_o_d_e_d_-_f_i_l_e_-_c_o_n_t_e_n_t. Note that the name is expected to be URL-encoded already.
       ...
       -d, --data <data>
              (HTTP  MQTT)  Sends  the  specified data in a POST request to the HTTP server, in the same way that a browser does when a user has filled in an HTML form and presses the submit button. This
              will cause curl to pass the data to the server using the content-type application/x-www-form-urlencoded. Compare to _-_F_, _-_-_f_o_r_m.

              _-_-_d_a_t_a_-_r_a_w is almost the same but does not have a special interpretation of the @ character. To post data purely binary, you should instead use the _-_-_d_a_t_a_-_b_i_n_a_r_y option. To  URL-encode  the
              value of a form field you may use _-_-_d_a_t_a_-_u_r_l_e_n_c_o_d_e.

              If  any  of  these  options is used more than once on the same command line, the data pieces specified will be merged with a separating &-symbol. Thus, using '-d name=daniel -d skill=lousy'
              would generate a post chunk that looks like 'name=daniel&skill=lousy'.

              If you start the data with the letter @, the rest should be a file name to read the data from, or - if you want curl to read the data from stdin. Posting data from  a  file  named  'foobar'
              would  thus be done with _-_d_, _-_-_d_a_t_a @foobar. When _-_d_, _-_-_d_a_t_a is told to read from a file like that, carriage returns and newlines will be stripped out. If you do not want the @ character to
              ...
              See also _-_-_d_a_t_a_-_b_i_n_a_r_y, _-_-_d_a_t_a_-_u_r_l_e_n_c_o_d_e and _-_-_d_a_t_a_-_r_a_w. This option overrides _-_F_, _-_-_f_o_r_m and _-_I_, _-_-_h_e_a_d and _-_T_, _-_-_u_p_l_o_a_d_-_f_i_l_e.
       ...
       --digest
              (HTTP) Enables HTTP Digest authentication. This is an authentication scheme that prevents the password from being sent over the wire in clear text. Use this in combination with  the  normal
              _-_u_, _-_-_u_s_e_r option to set user name and password.
              ...
              See also _-_u_, _-_-_u_s_e_r, _-_-_p_r_o_x_y_-_d_i_g_e_s_t and _-_-_a_n_y_a_u_t_h. This option overrides _-_-_b_a_s_i_c and _-_-_n_t_l_m and _-_-_n_e_g_o_t_i_a_t_e.
       ...
       --disable-eprt
              ...
              Disabling EPRT only changes the active behavior. If you want to switch to passive mode you need to not use _-_P_, _-_-_f_t_p_-_p_o_r_t or force it with _-_-_f_t_p_-_p_a_s_v.
       ...
       --disable-epsv
              (FTP)  Tell  curl to disable the use of the EPSV command when doing passive FTP transfers. Curl will normally always first attempt to use EPSV before PASV, but with this option, it will not
              try using EPSV.
              ...
              Disabling EPSV only changes the passive behavior. If you want to switch to active mode you need to use _-_P_, _-_-_f_t_p_-_p_o_r_t.
       ...
       -q, --disable
              If used as the first parameter on the command line, the _c_u_r_l_r_c config file will not be read and used. See the _-_K_, _-_-_c_o_n_f_i_g for details on the default config file search path.
       ...
       --dns-interface <interface>
              (DNS) Tell curl to send outgoing DNS requests through <interface>. This option is a counterpart to _-_-_i_n_t_e_r_f_a_c_e (which does not affect DNS). The supplied string must  be  an  interface  name
              (not an address).

              Example:
               curl --dns-interface eth0 https://example.com

              See also _-_-_d_n_s_-_i_p_v_4_-_a_d_d_r and _-_-_d_n_s_-_i_p_v_6_-_a_d_d_r. _-_-_d_n_s_-_i_n_t_e_r_f_a_c_e requires that the underlying libcurl was built to support c-ares. Added in 7.33.0.
       ...
       --dns-ipv4-addr <address>
              ...
              See also _-_-_d_n_s_-_i_n_t_e_r_f_a_c_e and _-_-_d_n_s_-_i_p_v_6_-_a_d_d_r. _-_-_d_n_s_-_i_p_v_4_-_a_d_d_r requires that the underlying libcurl was built to support c-ares. Added in 7.33.0.
       ...
       --dns-ipv6-addr <address>
              ...
              See also _-_-_d_n_s_-_i_n_t_e_r_f_a_c_e and _-_-_d_n_s_-_i_p_v_4_-_a_d_d_r. _-_-_d_n_s_-_i_p_v_6_-_a_d_d_r requires that the underlying libcurl was built to support c-ares. Added in 7.33.0.

       --dns-servers <addresses>
              Set  the list of DNS servers to be used instead of the system default.  The list of IP addresses should be separated with commas. Port numbers may also optionally be given as _:_<_p_o_r_t_-_n_u_m_b_e_r_>
              ...
              See also _-_-_d_n_s_-_i_n_t_e_r_f_a_c_e and _-_-_d_n_s_-_i_p_v_4_-_a_d_d_r. _-_-_d_n_s_-_s_e_r_v_e_r_s requires that the underlying libcurl was built to support c-ares. Added in 7.33.0.
       ...
       --doh-url <URL>
              ...
              Some SSL options that you set for your transfer will apply to DoH since the name lookups take place over SSL. However, the certificate verification settings are not  inherited  and  can  be
       ...
       --engine <name>
              (TLS)  Select the OpenSSL crypto engine to use for cipher operations. Use _-_-_e_n_g_i_n_e list to print a list of build-time supported engines. Note that not all (and possibly none) of the engines
       ...
       --etag-compare <file>
              ...
              Use the option _-_-_e_t_a_g_-_s_a_v_e to first save the ETag from a response, and then use this option to compare against the saved ETag in a subsequent request.
       ...
       --fail-early
              ...
              Using this option, curl will instead return an error on the first transfer that fails, independent of the amount of URLs that are given on the command line. This way, no  transfer  failures
              ...
              This  option  does  not  imply _-_f_, _-_-_f_a_i_l, which causes transfers to fail due to the server's HTTP status code. You can combine the two options, however note _-_f_, _-_-_f_a_i_l is not global and is
       ...
       --fail-with-body
              ...
              This is an alternative option to _-_f_, _-_-_f_a_i_l which makes curl fail for the same circumstances but without saving the content.

              Example:
               curl --fail-with-body https://example.com

              See also _-_f_, _-_-_f_a_i_l. Added in 7.76.0.

       -f, --fail
              (HTTP) Fail silently (no output at all) on server errors. This is mostly done to enable scripts etc to better deal with failed attempts. In normal cases when an HTTP server fails to deliver
              ...
              See also _-_-_f_a_i_l_-_w_i_t_h_-_b_o_d_y.

       --false-start
              (TLS) Tells curl to use false start during the TLS handshake. False start is a mode where a TLS client will start sending application data before verifying the  server's  Finished  message,
              ...
              See also _-_-_t_c_p_-_f_a_s_t_o_p_e_n. Added in 7.42.0.
       ...
       --form-string <name=string>
              (HTTP  SMTP  IMAP) Similar to _-_F_, _-_-_f_o_r_m except that the value string for the named parameter is used literally. Leading '@' and '<' characters, and the ';type=' string in the value have no
              special meaning. Use this in preference to _-_F_, _-_-_f_o_r_m if there's any possibility that the string value may accidentally trigger the '@' or '<' features of _-_F_, _-_-_f_o_r_m.
       ...
       -F, --form <name=content>
              ...
              This  enables  uploading  of  binary files etc. To force the 'content' part to be a file, prefix the file name with an @ sign. To just get the content part from a file, prefix the file name
              with the symbol <. The difference between @ and < is then that @ makes a file get attached in the post as a file upload, while the < makes a text field and just get the  contents  for  that
              ...
              Data  can be encoded for transfer using encoder=. Available encodings are _b_i_n_a_r_y and _8_b_i_t that do nothing else than adding the corresponding Content-Transfer-Encoding header, _7_b_i_t that only
              rejects 8-bit characters with a transfer error, _q_u_o_t_e_d_-_p_r_i_n_t_a_b_l_e and _b_a_s_e_6_4 that encodes data according to the corresponding schemes, limiting lines length to 76 characters.
              ...
              See also _-_d_, _-_-_d_a_t_a, _-_-_f_o_r_m_-_s_t_r_i_n_g and _-_-_f_o_r_m_-_e_s_c_a_p_e. This option overrides _-_d_, _-_-_d_a_t_a and _-_I_, _-_-_h_e_a_d and _-_T_, _-_-_u_p_l_o_a_d_-_f_i_l_e.
       ...
       --ftp-create-dirs
              (FTP  SFTP) When an FTP or SFTP URL/operation uses a path that does not currently exist on the server, the standard behavior of curl is to fail. Using this option, curl will instead attempt
       ...
       --ftp-method <method>
              ...
              singlecwd
                     curl  does  one  CWD  with  the full target directory and then operates on the file "normally" (like in the multicwd case). This is somewhat more standards compliant than 'nocwd' but
       ...
       --ftp-pasv
              (FTP) Use passive mode for the data connection. Passive is the internal default behavior, but using this option can be used to override a previous _-_P_, _-_-_f_t_p_-_p_o_r_t option.

              If this option is used several times, only the first one is used. Undoing an enforced passive really is not doable but you must then instead enforce the correct _-_P_, _-_-_f_t_p_-_p_o_r_t again.
       ...
       -P, --ftp-port <address>
              (FTP) Reverses the default initiator/listener roles when connecting with FTP. This option makes curl use active mode. curl then tells the server to connect back to  the  client's  specified
              address and port, while passive mode asks the server to setup an IP address and port for it to connect to. <address> should be one of:
       ...
       If  this option is used several times, the last one will be used. Disable the use of PORT with _-_-_f_t_p_-_p_a_s_v. Disable the attempt to use the EPRT command instead of PORT by using _-_-_d_i_s_a_b_l_e_-_e_p_r_t. EPRT
       is really PORT++.

       You can also append ":[start]-[end]" to the right of the address, to tell curl what TCP port range to use. That means you specify a port range, from a lower to a higher  number.  A  single  number
       ...
       --ftp-pret
              (FTP)  Tell  curl  to send a PRET command before PASV (and EPSV). Certain FTP servers, mainly drftpd, require this non-standard command for directory listings as well as up and downloads in
              PASV mode.
       ...
       --ftp-ssl-ccc-mode <active/passive>
              (FTP)  Sets the CCC mode. The passive mode will not initiate the shutdown, but instead wait for the server to do it, and will not reply to the shutdown from the server. The active mode ini‐
       ...
       --ftp-ssl-control
              (FTP)  Require SSL/TLS for the FTP login, clear for transfer.  Allows secure authentication, but non-encrypted data transfers for efficiency.  Fails the transfer if the server does not sup‐
              port SSL/TLS.
       ...
       -G, --get
              When used, this option will make all data specified with _-_d_, _-_-_d_a_t_a, _-_-_d_a_t_a_-_b_i_n_a_r_y or _-_-_d_a_t_a_-_u_r_l_e_n_c_o_d_e to be used in an HTTP GET request instead of the POST request that otherwise would  be
       ...
       --happy-eyeballs-timeout-ms <milliseconds>
              Happy Eyeballs is an algorithm that attempts to connect to both IPv4 and IPv6 addresses for dual-stack hosts, giving IPv6 a head-start of the specified number of milliseconds. If  the  IPv6
              address cannot be connected to within that time, then a connection attempt is made to the IPv4 address in parallel. The first connection to be established is the one that is used.
       ...
       -I, --head
              (HTTP FTP FILE) Fetch the headers only! HTTP-servers feature the command HEAD which this uses to get nothing but the header of a document. When used on an FTP or FILE  file,  curl  displays
       ...
       -H, --header <header/@file>
              (HTTP)  Extra header to include in the request when sending HTTP to a server. You may specify any number of extra headers. Note that if you should add a custom header that has the same name
              as one of the internal ones curl would use, your externally set header will be used instead of the internal one. This allows you to make even trickier stuff than curl would normally do. You
              should  not  replace  internally  set  headers  without knowing perfectly well what you are doing. Remove an internal header by giving a replacement without content on the right side of the
              colon, as in: -H "Host:". If you send the custom header with no-value then its header must be terminated with a semicolon, such as -H "X-Custom-Header;" to send "X-Custom-Header:".

              curl will make sure that each header you add/replace is sent with the proper end-of-line marker, you should thus nnoott add that as a part of the header content: do not add  newlines  or  car‐
              ...
              You need _-_-_p_r_o_x_y_-_h_e_a_d_e_r to send custom headers intended for a HTTP proxy. Added in 7.37.0.
              ...
              WWAARRNNIINNGG:  headers  set  with this option will be set in all requests - even after redirects are followed, like when told with _-_L_, _-_-_l_o_c_a_t_i_o_n. This can lead to the header being sent to other
       ...
       --hostpubmd5 <md5>
              (SFTP  SCP)  Pass a string containing 32 hexadecimal digits. The string should be the 128 bit MD5 checksum of the remote host's public key, curl will refuse the connection with the host un‐
              less the md5sums match.
       ...
       --http0.9
              ...
              HTTP/0.9  is  a  completely headerless response and therefore you can also connect with this to non-HTTP servers and still get a response since curl will simply transparently downgrade - if
              allowed.
       ...
       -0, --http1.0
              ...
              See also _-_-_h_t_t_p_0_._9 and _-_-_h_t_t_p_1_._1. This option overrides _-_-_h_t_t_p_1_._1 and _-_-_h_t_t_p_2.
       ...
       --http1.1
              ...
              See also _-_-_h_t_t_p_1_._1 and _-_-_h_t_t_p_0_._9. This option overrides _-_0_, _-_-_h_t_t_p_1_._0 and _-_-_h_t_t_p_2. Added in 7.33.0.
       ...
       --http2-prior-knowledge
              ...
              See  also _-_-_h_t_t_p_2 and _-_-_h_t_t_p_3. _-_-_h_t_t_p_2_-_p_r_i_o_r_-_k_n_o_w_l_e_d_g_e requires that the underlying libcurl was built to support HTTP/2. This option overrides _-_-_h_t_t_p_1_._1 and _-_0_, _-_-_h_t_t_p_1_._0 and _-_-_h_t_t_p_2. Added
              in 7.49.0.
       ...
       --http2
              ...
              See also _-_-_h_t_t_p_1_._1 and _-_-_h_t_t_p_3. _-_-_h_t_t_p_2 requires that the underlying libcurl was built to support HTTP/2. This option overrides _-_-_h_t_t_p_1_._1  and  _-_0_,  _-_-_h_t_t_p_1_._0  and  _-_-_h_t_t_p_2_-_p_r_i_o_r_-_k_n_o_w_l_e_d_g_e.
              Added in 7.33.0.

       --http3
              (HTTP) WWAARRNNIINNGG: this option is experimental. Do not use in production.

              Tells  curl  to use HTTP version 3 directly to the host and port number used in the URL. A normal HTTP/3 transaction will be done to a host and then get redirected via Alt-Svc, but this op‐
              tion allows a user to circumvent that when you know that the target speaks HTTP/3 on the given host and port.
              ...
              See also _-_-_h_t_t_p_1_._1 and _-_-_h_t_t_p_2. _-_-_h_t_t_p_3 requires that the underlying libcurl was built to support HTTP/3. This option overrides _-_-_h_t_t_p_1_._1 and _-_0_, _-_-_h_t_t_p_1_._0 and  _-_-_h_t_t_p_2  and  _-_-_h_t_t_p_2_-_p_r_i_o_r_-
              _k_n_o_w_l_e_d_g_e. Added in 7.66.0.
       ...
       -k, --insecure
              ...
              When  this  option  is  not used for protocols using TLS, curl verifies the server's TLS certificate before it continues: that the certificate contains the right name which matches the host
              name used in the URL and that the certificate has been signed by a CA certificate present in the cert store.  See this online resource for further details:
               https://curl.se/docs/sslcerts.html

              For SFTP and SCP, this option makes curl skip the _k_n_o_w_n___h_o_s_t_s verification.  _k_n_o_w_n___h_o_s_t_s is a file normally stored in the user's home directory in the .ssh subdirectory, which contains host
       ...
       --interface <name>
              ...
              On  Linux  it  can  be  used  to specify a VRF, but the binary needs to either have CAP_NET_RAW or to be run as root. More information about Linux VRF: https://www.kernel.org/doc/Documenta‐
       ...
       --keepalive-time <seconds>
              ...
              offering the TCP_KEEPIDLE and TCP_KEEPINTVL socket options (meaning Linux, recent AIX, HP-UX and more). This option has no effect if _-_-_n_o_-_k_e_e_p_a_l_i_v_e is used.
       ...
       --key-type <type>
              (TLS) Private key file type. Specify which type your _-_-_k_e_y provided private key is. DER, PEM, and ENG are supported. If not specified, PEM is assumed.
       ...
       --key <key>
              ...
              ning with "pkcs11:" will be interpreted as a PKCS#11 URI. If a PKCS#11 URI is provided, then the _-_-_e_n_g_i_n_e option will be set as "pkcs11" if none was provided and the _-_-_k_e_y_-_t_y_p_e option  will
              be set as "ENG" if none was provided.
       ...
       --krb <level>
              ...
              See also _-_-_d_e_l_e_g_a_t_i_o_n and _-_-_s_s_l. _-_-_k_r_b requires that the underlying libcurl was built to support Kerberos.
       ...
       --limit-rate <speed>
              Specify the maximum transfer rate you want curl to use - for both downloads and uploads. This feature is useful if you have a limited pipe and you would like your transfer not to  use  your
              ...
              If you also use the _-_Y_, _-_-_s_p_e_e_d_-_l_i_m_i_t option, that option will take precedence and might cripple the rate-limiting slightly, to help keeping the speed-limit logic working.
              ...
              See also _-_Y_, _-_-_s_p_e_e_d_-_l_i_m_i_t and _-_y_, _-_-_s_p_e_e_d_-_t_i_m_e.
       ...
       -l, --list-only
              ...
              Note: When combined with _-_X_, _-_-_r_e_q_u_e_s_t, this option can be used to send a UIDL command instead, so the user may use the email's unique identifier rather than its message-id to make the  re‐
              quest.
       ...
       --local-port <num/range>
              Set  a  preferred  single number or range (FROM-TO) of local port numbers to use for the connection(s).  Note that port numbers by nature are a scarce resource that will be busy at times so
              setting this range to something too narrow might cause unnecessary connection setup failures.
       ...
       --location-trusted
              (HTTP) Like _-_L_, _-_-_l_o_c_a_t_i_o_n, but will allow sending the name + password to all hosts that the site may redirect to. This may or may not introduce a security breach if the site redirects  you
              to a site to which you will send your authentication info (which is plaintext in the case of HTTP Basic authentication).
       ...
       -L, --location
              ...
              quest on the new place. If used together with _-_i_, _-_-_i_n_c_l_u_d_e or _-_I_, _-_-_h_e_a_d, headers from all requested pages will be shown. When authentication is used, curl only sends  its  credentials  to
              the  initial  host.  If  a  redirect takes curl to a different host, it will not be able to intercept the user+password. See also _-_-_l_o_c_a_t_i_o_n_-_t_r_u_s_t_e_d on how to change this. You can limit the
              amount of redirects to follow by using the _-_-_m_a_x_-_r_e_d_i_r_s option.
              ...
              You can tell curl to not change POST requests to GET after a 30x response by using the dedicated options for that: _-_-_p_o_s_t_3_0_1, _-_-_p_o_s_t_3_0_2 and _-_-_p_o_s_t_3_0_3.

              The method set with _-_X_, _-_-_r_e_q_u_e_s_t overrides the method curl would otherwise select to use.
       ...
       --login-options <options>
              ...
              See also _-_u_, _-_-_u_s_e_r. Added in 7.34.0.
       ...
       --mail-rcpt-allowfails
              ...
              The default behavior can be changed by passing _-_-_m_a_i_l_-_r_c_p_t_-_a_l_l_o_w_f_a_i_l_s command-line option which will make curl ignore errors and proceed with the remaining valid recipients.
       ...
       --max-filesize <bytes>
              (FTP HTTP MQTT) Specify the maximum size (in bytes) of a file to download. If the file requested is larger than this value, the transfer will not start and curl will return with  exit  code
              63.
              ...
              NNOOTTEE: The file size is not always known prior to download, and for such files this option has no effect even if the file transfer ends up being larger than this given limit.  Example:
               curl --max-filesize 100K https://example.com

              See also _-_-_l_i_m_i_t_-_r_a_t_e.

       --max-redirs <num>
              (HTTP) Set maximum number of redirections to follow. When _-_L_, _-_-_l_o_c_a_t_i_o_n is used, to prevent curl from following too many redirects, by default, the limit is set to 50 redirects.  Set  this
       ...
       --negotiate
              ...
              This option requires a library built with GSS-API or SSPI support. Use _-_V_, _-_-_v_e_r_s_i_o_n to see if your curl supports GSS-API/SSPI or SPNEGO.

              When using this option, you must also provide a fake _-_u_, _-_-_u_s_e_r option to activate the authentication code properly. Sending a '-u :' is enough as the user name and password  from  the  _-_u_,
       ...
       --netrc-file <filename>
              This option is similar to _-_n_, _-_-_n_e_t_r_c, except that you provide the path (absolute or relative) to the netrc file that curl should use. You can only specify one netrc file per invocation. If
              several _-_-_n_e_t_r_c_-_f_i_l_e options are provided, the last one will be used.
              ...
              See also _-_n_, _-_-_n_e_t_r_c, _-_u_, _-_-_u_s_e_r and _-_K_, _-_-_c_o_n_f_i_g. This option overrides _-_n_, _-_-_n_e_t_r_c.

       --netrc-optional
              Similar to _-_n_, _-_-_n_e_t_r_c, but this option makes the .netrc usage ooppttiioonnaall and not mandatory as the _-_n_, _-_-_n_e_t_r_c option does.
       ...
       -n, --netrc
              Makes curl scan the _._n_e_t_r_c (___n_e_t_r_c on Windows) file in the user's home directory for login name and password. This is typically used for FTP on Unix. If used with  HTTP,  curl  will  enable
              user  authentication. See _n_e_t_r_c_(_5_) and _f_t_p_(_1_) for details on the file format. Curl will not complain if that file does not have the right permissions (it should be neither world- nor group-
              readable). The environment variable "HOME" is used to find the home directory.

              A quick and simple example of how to setup a _._n_e_t_r_c to allow curl to FTP to the machine host.domain.com with user name 'myself' and password 'secret' could look similar to:
              ...
              See also _-_-_n_e_t_r_c_-_f_i_l_e, _-_K_, _-_-_c_o_n_f_i_g and _-_u_, _-_-_u_s_e_r.
       ...
       -:, --next
              ...
              _-_:_, _-_-_n_e_x_t will reset all local options and only global ones will have their values survive over to the operation following the _-_:_, _-_-_n_e_x_t instruction. Global options include _-_v_, _-_-_v_e_r_b_o_s_e,
              ...
              See also _-_Z_, _-_-_p_a_r_a_l_l_e_l and _-_K_, _-_-_c_o_n_f_i_g. Added in 7.36.0.
       ...
       --no-alpn
              ...
              See also _-_-_n_o_-_n_p_n and _-_-_h_t_t_p_2. _-_-_n_o_-_a_l_p_n requires that the underlying libcurl was built to support TLS. Added in 7.36.0.
       ...
       --no-npn
              (HTTPS) Disable the NPN TLS extension. NPN is enabled by default if libcurl was built with an SSL library that supports NPN. NPN is used by a  libcurl  that  supports  HTTP/2  to  negotiate
              HTTP/2 support with the server during https sessions.

              Example:
               curl --no-npn https://example.com

              See also _-_-_n_o_-_a_l_p_n and _-_-_h_t_t_p_2. _-_-_n_o_-_n_p_n requires that the underlying libcurl was built to support TLS. Added in 7.36.0.

       --no-progress-meter
              Option to switch off the progress meter output without muting or otherwise affecting warning and informational messages like _-_s_, _-_-_s_i_l_e_n_t does.
       ...
       --noproxy <no-proxy-list>
              ...
              Since 7.53.0, This option overrides the environment variables that disable the proxy ('no_proxy' and 'NO_PROXY'). If there's an environment variable disabling a proxy, you can set  the  no‐
       ...
       --ntlm-wb
              (HTTP) Enables NTLM much in the style _-_-_n_t_l_m does, but hand over the authentication to the separate binary ntlmauth application that is executed when needed.
       ...
       --ntlm (HTTP)  Enables NTLM authentication. The NTLM authentication method was designed by Microsoft and is used by IIS web servers. It is a proprietary protocol, reverse-engineered by clever peo‐
              ple and implemented in curl based on their efforts. This kind of behavior should not be endorsed, you should encourage everyone who uses NTLM to switch to a public and documented  authenti‐
              cation method instead, such as Digest.
              ...
              See also _-_-_p_r_o_x_y_-_n_t_l_m. _-_-_n_t_l_m requires that the underlying libcurl was built to support TLS. This option overrides _-_-_b_a_s_i_c and _-_-_n_e_g_o_t_i_a_t_e and _-_-_d_i_g_e_s_t and _-_-_a_n_y_a_u_t_h.

       --oauth2-bearer <token>
              (IMAP POP3 SMTP HTTP) Specify the Bearer Token for OAUTH 2.0 server authentication. The Bearer Token is used in conjunction with the user name which can be specified as part of the _-_-_u_r_l or
              _-_u_, _-_-_u_s_e_r options.
              ...
              See also _-_-_b_a_s_i_c, _-_-_n_t_l_m and _-_-_d_i_g_e_s_t. Added in 7.33.0.

       --output-dir <dir>

              This option specifies the directory in which files should be stored, when _-_O_, _-_-_r_e_m_o_t_e_-_n_a_m_e or _-_o_, _-_-_o_u_t_p_u_t are used.

              The given output directory is used for all URLs and output options on the command line, up until the first _-_:_, _-_-_n_e_x_t.

              If the specified target directory does not exist, the operation will fail unless _-_-_c_r_e_a_t_e_-_d_i_r_s is also used.
              ...
              See also _-_O_, _-_-_r_e_m_o_t_e_-_n_a_m_e and _-_J_, _-_-_r_e_m_o_t_e_-_h_e_a_d_e_r_-_n_a_m_e. Added in 7.73.0.
       ...
       -o, --output <file>
              ...
              See also the _-_-_c_r_e_a_t_e_-_d_i_r_s option to create the local directories dynamically. Specifying the output as '-' (a single dash) will force the output to be done to stdout.
              ...
              See also _-_O_, _-_-_r_e_m_o_t_e_-_n_a_m_e, _-_-_r_e_m_o_t_e_-_n_a_m_e_-_a_l_l and _-_J_, _-_-_r_e_m_o_t_e_-_h_e_a_d_e_r_-_n_a_m_e.
       ...
       --parallel-max <num>
              When asked to do parallel transfers, using _-_Z_, _-_-_p_a_r_a_l_l_e_l, this option controls the maximum amount of transfers to do simultaneously.
       ...
       --path-as-is
              ...
              See also _-_-_r_e_q_u_e_s_t_-_t_a_r_g_e_t. Added in 7.42.0.
       ...
       --pinnedpubkey <hashes>
              ...
              See also _-_-_h_o_s_t_p_u_b_s_h_a_2_5_6. Added in 7.39.0.

       --post301
              (HTTP) Tells curl to respect RFC 7231/6.4.2 and not convert POST requests into GET requests when following a 301 redirection. The non-RFC behavior is ubiquitous in  web  browsers,  so  curl
              does the conversion by default to maintain consistency. However, a server may require a POST to remain a POST after such a redirection. This option is meaningful only when using _-_L_, _-_-_l_o_c_a_‐
              _t_i_o_n.
       ...
       --post302
              (HTTP) Tells curl to respect RFC 7231/6.4.3 and not convert POST requests into GET requests when following a 302 redirection. The non-RFC behavior is ubiquitous in  web  browsers,  so  curl
              does the conversion by default to maintain consistency. However, a server may require a POST to remain a POST after such a redirection. This option is meaningful only when using _-_L_, _-_-_l_o_c_a_‐
              _t_i_o_n.
       ...
       --post303
              (HTTP) Tells curl to violate RFC 7231/6.4.4 and not convert POST requests into GET requests when following 303 redirections. A server may require a POST to remain a POST after a  303  redi‐
              rection. This option is meaningful only when using _-_L_, _-_-_l_o_c_a_t_i_o_n.
       ...
       --preproxy [protocol://]host[:port]
              Use the specified SOCKS proxy before connecting to an HTTP or HTTPS _-_x_, _-_-_p_r_o_x_y. In such a case curl first connects to the SOCKS proxy and then connects (through SOCKS) to the HTTP or HTTPS
       ...
       --proto-default <protocol>
              ...
              Without this option set, curl guesses protocol based on the host name, see _-_-_u_r_l for details.

              Example:
               curl --proto-default https ftp.example.com

              See also _-_-_p_r_o_t_o and _-_-_p_r_o_t_o_-_r_e_d_i_r. Added in 7.45.0.

       --proto-redir <protocols>
              Tells curl to limit what protocols it may use on redirect. Protocols denied by _-_-_p_r_o_t_o are not overridden by this option. See --proto for how protocols are represented.

              Example, allow only HTTP and HTTPS on redirect:

               curl --proto-redir -all,http,https http://example.com

              By default curl will only allow HTTP, HTTPS, FTP and FTPS on redirect (since 7.65.2). Specifying _a_l_l or _+_a_l_l enables all protocols on redirects, which is not good for security.
       ...
       --proto <protocols>
              Tells curl to limit what protocols it may use for transfers. Protocols are evaluated left to right, are comma separated, and are each a protocol name or 'all', optionally prefixed  by  zero
              ...
              Unknown  protocols produce a warning. This allows scripts to safely rely on being able to disable potentially dangerous protocols, without relying upon support for that protocol being built
       ...
       --proxy-basic
              Tells curl to use HTTP Basic authentication when communicating with the given proxy. Use _-_-_b_a_s_i_c for enabling HTTP Basic with a remote host. Basic is the default authentication method  curl
              uses with proxies.
       ...
       --proxy-cacert <file>
              ...
              See also _-_-_p_r_o_x_y_-_c_a_p_a_t_h, _-_-_c_a_c_e_r_t, _-_-_c_a_p_a_t_h and _-_x_, _-_-_p_r_o_x_y. Added in 7.52.0.
       ...
       --proxy-capath <dir>
              ...
              See also _-_-_p_r_o_x_y_-_c_a_c_e_r_t, _-_x_, _-_-_p_r_o_x_y and _-_-_c_a_p_a_t_h. Added in 7.52.0.
       ...
       --proxy-digest
              Tells curl to use HTTP Digest authentication when communicating with the given proxy. Use _-_-_d_i_g_e_s_t for enabling HTTP Digest with a remote host.
       ...
       --proxy-header <header/@file>
              (HTTP)  Extra header to include in the request when sending HTTP to a proxy. You may specify any number of extra headers. This is the equivalent option to _-_H_, _-_-_h_e_a_d_e_r but is for proxy com‐
              munication only like in CONNECT requests when you want a separate header sent to the proxy to what is sent to the actual remote host.
              ...
              See also _-_x_, _-_-_p_r_o_x_y. Added in 7.37.0.
       ...
       --proxy-negotiate
              Tells curl to use HTTP Negotiate (SPNEGO) authentication when communicating with the given proxy. Use _-_-_n_e_g_o_t_i_a_t_e for enabling HTTP Negotiate (SPNEGO) with a remote host.
       ...
       --proxy-ntlm
              Tells curl to use HTTP NTLM authentication when communicating with the given proxy. Use _-_-_n_t_l_m for enabling NTLM with a remote host.
       ...
       --proxy-service-name <name>
              ...
              See also _-_-_s_e_r_v_i_c_e_-_n_a_m_e and _-_x_, _-_-_p_r_o_x_y. Added in 7.43.0.
       ...
       --proxy-ssl-auto-client-cert
              ...
              See also _-_-_s_s_l_-_a_u_t_o_-_c_l_i_e_n_t_-_c_e_r_t and _-_x_, _-_-_p_r_o_x_y. Added in 7.77.0.
       ...
       --proxy-tls13-ciphers <ciphersuite list>
              ...
              This option is currently used only when curl is built to use OpenSSL 1.1.1 or later. If you are using a different SSL backend you can try setting TLS 1.3 cipher suites by using the _-_-_p_r_o_x_y_-
              _c_i_p_h_e_r_s option.
              ...
              See also _-_-_t_l_s_1_3_-_c_i_p_h_e_r_s and _-_-_c_u_r_v_e_s. Added in 7.61.0.
       ...
       -U, --proxy-user <user:password>
              ...
              If  you use a Windows SSPI-enabled curl binary and do either Negotiate or NTLM authentication then you can tell curl to select the user name and password from your environment by specifying
              a single colon with this option: "-U :".

              On systems where it works, curl will hide the given option argument from process listings. This is not enough to protect credentials from possibly getting seen by other users  on  the  same
              system as they will still be visible for a moment before cleared. Such sensitive data should be retrieved from a file instead or similar and never used in clear text in a command line.
       ...
       -x, --proxy [protocol://]host[:port]
              ...
              if you can tunnel through the proxy, as one with the _-_p_, _-_-_p_r_o_x_y_t_u_n_n_e_l option.
       ...
       --proxy1.0 <host[:port]>
              ...
              The only difference between this and the HTTP proxy option _-_x_, _-_-_p_r_o_x_y, is that attempts to use CONNECT through the proxy will specify an HTTP 1.0 protocol instead of the default HTTP 1.1.
       ...
       -p, --proxytunnel
              When  an HTTP proxy is used _-_x_, _-_-_p_r_o_x_y, this option will make curl tunnel through the proxy. The tunnel approach is made with the HTTP proxy CONNECT request and requires that the proxy al‐
              lows direct connect to the remote port number curl wants to tunnel through to.
       ...
       --pubkey <key>
              ...
              (As of 7.39.0, curl attempts to automatically extract the public key from the private key file, so passing this option is generally not required. Note that this public  key  extraction  re‐
              quires libcurl to be linked against a copy of libssh2 1.2.8 or higher that is itself linked against OpenSSL.)
       ...
       -Q, --quote <command>
              ...
              SFTP is a binary protocol. Unlike for FTP, curl interprets SFTP quote commands itself before sending them to the server. File names may be quoted shell-style  to  embed  spaces  or  special
              characters. Following is the list of all supported SFTP quote commands:

              atime date file
                     The  atime  command sets the last access time of the file named by the file operand. The <date expression> can be all sorts of date strings, see the _c_u_r_l___g_e_t_d_a_t_e_(_3_) man page for date
              ...
              mtime date file
                     The mtime command sets the last modification time of the file named by the file operand. The <date expression> can be all sorts of date strings, see the _c_u_r_l___g_e_t_d_a_t_e_(_3_) man page  for
       ...
       --random-file <file>
              Specify the path name to file containing what will be considered as random data. The data may be used to seed the random engine for SSL connections. See also the _-_-_e_g_d_-_f_i_l_e option.
       ...
       -r, --range <range>
              ...
              Only digit characters (0-9) are valid in the 'start' and 'stop' fields of the 'start-stop' range syntax. If a non-digit character is given in the range, the server's response  will  be  un‐
              specified, depending on the server's configuration.
       ...
       -e, --referer <URL>
              (HTTP)  Sends  the "Referrer Page" information to the HTTP server. This can also be set with the _-_H_, _-_-_h_e_a_d_e_r flag of course. When used with _-_L_, _-_-_l_o_c_a_t_i_o_n you can append ";auto" to the _-_e_,
              _-_-_r_e_f_e_r_e_r URL to make curl automatically set the previous URL when it follows a Location: header. The ";auto" string can be used alone, even if you do not set an initial _-_e_, _-_-_r_e_f_e_r_e_r.
       ...
       -J, --remote-header-name
              (HTTP) This option tells the _-_O_, _-_-_r_e_m_o_t_e_-_n_a_m_e option to use the server-specified Content-Disposition filename instead of extracting a filename from the URL.

              If the server specifies a file name and a file with that name already exists in the current working directory it will not be overwritten and an error will occur.  If  the  server  does  not
              ...
              WWAARRNNIINNGG:  Exercise  judicious  use of this option, especially on Windows. A rogue server could send you the name of a DLL or other file that could be loaded automatically by Windows or some
       ...
       --remote-name-all
              This option changes the default action for all given URLs to be dealt with as if _-_O_, _-_-_r_e_m_o_t_e_-_n_a_m_e were used for each one. So if you want to disable that for a specific URL after  _-_-_r_e_m_o_t_e_-
              _n_a_m_e_-_a_l_l has been used, you must use "-o -" or --no-remote-name.
       ...
       -O, --remote-name
              ...
              name refer to _-_J_, _-_-_r_e_m_o_t_e_-_h_e_a_d_e_r_-_n_a_m_e which can be used in addition to this option. If the server chooses a file name and that name already exists it will not be overwritten.
              ...
              See also _-_-_r_e_m_o_t_e_-_n_a_m_e_-_a_l_l.
       ...
       -X, --request <method>
              ...
              suffice. You need to use the _-_I_, _-_-_h_e_a_d option.

              The  method  string you set with _-_X_, _-_-_r_e_q_u_e_s_t will be used for all requests, which if you for example use _-_L_, _-_-_l_o_c_a_t_i_o_n may cause unintended side-effects when curl does not change request
       ...
       --resolve <[+]host:port:addr[,addr]...>
              ...
              By specifying '*' as host you can tell curl to resolve any host and specific port pair to the specified address. Wildcard is resolved last so any _-_-_r_e_s_o_l_v_e with a  specific  host  and  port
              will be used first.

              The provided address set by this option will be used even if _-_4_, _-_-_i_p_v_4 or _-_6_, _-_-_i_p_v_6 is set to make curl use another IP version.

              By  prefixing the host with a '+' you can make the entry time out after curl's default timeout (1 minute). Note that this will only make sense for long running parallel transfers with a lot
              of files. In such cases, if this option is used curl will try to resolve the host as it normally would once the timeout has expired.
       ...
       --retry-all-errors
              ...
              WWAARRNNIINNGG:  For server compatibility curl attempts to retry failed flaky transfers as close as possible to how they were started, but this is not possible with redirected input or output. For
              example, before retrying it removes output data from a failed partial transfer that was written to an output file. However this is not true of data redirected to a | pipe or >  file,  which
              are not reset. We strongly suggest you do not parse or record output via redirect in combination with this option, since you may receive duplicate data.

              By  default  curl will not error on an HTTP response code that indicates an HTTP error, if the transfer was successful. For example, if a server replies 404 Not Found and the reply is fully
              received then that is not an error. When _-_-_r_e_t_r_y is used then curl will retry on some HTTP response codes that indicate transient HTTP errors, but that does not include  most  4xx  response
              codes such as 404. If you want to retry on all response codes that indicate HTTP errors (4xx and 5xx) then combine with _-_f_, _-_-_f_a_i_l.
       ...
       --retry-connrefused
              In addition to the other conditions, consider ECONNREFUSED as a transient error too for _-_-_r_e_t_r_y. This option is used together with --retry.
       ...
       --retry-delay <seconds>
              ...
              interesting if _-_-_r_e_t_r_y is also used. Setting this delay to zero will make curl use the default backoff time.
       ...
       --retry-max-time <seconds>
              The retry timer is reset before the first transfer attempt. Retries will be done as usual (see _-_-_r_e_t_r_y) as long as the timer has not reached this given limit. Notice that if the  timer  has
              not  reached the limit, the request will be made and while performing, it may take longer than this given time period. To limit a single request's maximum time, use _-_m_, _-_-_m_a_x_-_t_i_m_e. Set this
       ...
       --retry <num>
              ...
              delay between the rest of the retries. By using _-_-_r_e_t_r_y_-_d_e_l_a_y you disable this exponential backoff algorithm. See also _-_-_r_e_t_r_y_-_m_a_x_-_t_i_m_e to limit the total time allowed for retries.
       ...
       --sasl-authzid <identity>
              Use this authorisation identity (authzid), during SASL PLAIN authentication, in addition to the authentication identity (authcid) as specified by _-_u_, _-_-_u_s_e_r.

              If the option is not specified, the server will derive the authzid from the authcid, but if specified, and depending on the server implementation, it may be used to  access  another  user's
       ...
       --sasl-ir
              ...
              See also _-_-_s_a_s_l_-_a_u_t_h_z_i_d. Added in 7.31.0.

       --service-name <name>
              This option allows you to change the service name for SPNEGO.

              Examples: _-_-_n_e_g_o_t_i_a_t_e _-_-_s_e_r_v_i_c_e_-_n_a_m_e sockd would use sockd/server-name.

              Example:
               curl --service-name sockd/server https://example.com

              See also _-_-_n_e_g_o_t_i_a_t_e and _-_-_p_r_o_x_y_-_s_e_r_v_i_c_e_-_n_a_m_e. Added in 7.43.0.

       -S, --show-error
              When used with _-_s_, _-_-_s_i_l_e_n_t, it makes curl show an error message if it fails.
       ...
       -s, --silent
              Silent  or  quiet mode. Do not show progress meter or error messages. Makes Curl mute. It will still output the data you ask for, potentially even to the terminal/stdout unless you redirect
              it.

              Use _-_S_, _-_-_s_h_o_w_-_e_r_r_o_r in addition to this option to disable progress meter but still show error messages.
       ...
       --socks4 <host[:port]>
              ...
              This option is superfluous since you can specify a socks4 proxy with _-_x_, _-_-_p_r_o_x_y using a socks4:// protocol prefix.

              Since  7.52.0,  _-_-_p_r_e_p_r_o_x_y can be used to specify a SOCKS proxy at the same time _-_x_, _-_-_p_r_o_x_y is used with an HTTP/HTTPS proxy. In such a case curl first connects to the SOCKS proxy and then
       ...
       --socks4a <host[:port]>
              ...
              This option is superfluous since you can specify a socks4a proxy with _-_x_, _-_-_p_r_o_x_y using a socks4a:// protocol prefix.
       ...
       --socks5-basic
              Tells  curl to use username/password authentication when connecting to a SOCKS5 proxy.  The username/password authentication is enabled by default.  Use _-_-_s_o_c_k_s_5_-_g_s_s_a_p_i to force GSS-API au‐
       ...
       --socks5-gssapi-nec
              ...
              _-_-_s_o_c_k_s_5_-_g_s_s_a_p_i_-_n_e_c allows the unprotected exchange of the protection mode negotiation.
       ...
       --socks5-gssapi-service <name>
              ...
              Examples:  _-_-_s_o_c_k_s_5 proxy-name _-_-_s_o_c_k_s_5_-_g_s_s_a_p_i_-_s_e_r_v_i_c_e sockd would use sockd/proxy-name _-_-_s_o_c_k_s_5 proxy-name _-_-_s_o_c_k_s_5_-_g_s_s_a_p_i_-_s_e_r_v_i_c_e sockd/real-name would use sockd/real-name for cases where
       ...
       --socks5-gssapi
              Tells curl to use GSS-API authentication when connecting to a SOCKS5 proxy.  The GSS-API authentication is enabled by default (if curl is compiled with GSS-API support).  Use _-_-_s_o_c_k_s_5_-_b_a_s_i_c
       ...
       --socks5-hostname <host[:port]>
              ...
              This option is superfluous since you can specify a socks5 hostname proxy with _-_x_, _-_-_p_r_o_x_y using a socks5h:// protocol prefix.
       ...
       --socks5 <host[:port]>
              ...
              This option is superfluous since you can specify a socks5 proxy with _-_x_, _-_-_p_r_o_x_y using a socks5:// protocol prefix.
       ...
       -Y, --speed-limit <speed>
              If a download is slower than this given speed (in bytes per second) for speed-time seconds it gets aborted. speed-time is set with _-_y_, _-_-_s_p_e_e_d_-_t_i_m_e and is 30 if not set.
       ...
       -y, --speed-time <seconds>
              If a download is slower than speed-limit bytes per second during a speed-time period, the download gets aborted. If speed-time is used, the default speed-limit will be 1 unless set with _-_Y_,
              _-_-_s_p_e_e_d_-_l_i_m_i_t.

              This option controls transfers and thus will not affect slow connects etc. If this is a concern for you, try the _-_-_c_o_n_n_e_c_t_-_t_i_m_e_o_u_t option.
       ...
       --ssl-allow-beast
              This option tells curl to not work around a security flaw in the SSL3 and TLS1.0 protocols known as BEAST.  If this option is not used, the SSL layer may use workarounds known to cause  in‐
              teroperability problems with some older SSL implementations.

              WWAARRNNIINNGG: this option loosens the SSL security, and by using this flag you ask for exactly that.
       ...
       --ssl-auto-client-cert
              ...
              brary). Prior to 7.77.0 this was the default behavior in libcurl with Schannel. Since the server can request any certificate that supports client authentication in the OS certificate  store
              it could be a privacy violation and unexpected.
       ...
       --ssl-no-revoke
              ...
              See also _-_-_c_r_l_f_i_l_e. Added in 7.44.0.
       ...
       --ssl  (FTP  IMAP  POP3  SMTP  LDAP) Try to use SSL/TLS for the connection. Reverts to a non-secure connection if the server does not support SSL/TLS. See also _-_-_f_t_p_-_s_s_l_-_c_o_n_t_r_o_l and _-_-_s_s_l_-_r_e_q_d for
              ...
              See also _-_k_, _-_-_i_n_s_e_c_u_r_e and _-_-_c_i_p_h_e_r_s.
       ...
       -2, --sslv2
              ...
              See also _-_-_h_t_t_p_1_._1 and _-_-_h_t_t_p_2. _-_2_, _-_-_s_s_l_v_2 requires that the underlying libcurl was built to support TLS. This option overrides _-_3_, _-_-_s_s_l_v_3 and _-_1_, _-_-_t_l_s_v_1 and _-_-_t_l_s_v_1_._1 and _-_-_t_l_s_v_1_._2.
       ...
       -3, --sslv3
              ...
              See also _-_-_h_t_t_p_1_._1 and _-_-_h_t_t_p_2. _-_3_, _-_-_s_s_l_v_3 requires that the underlying libcurl was built to support TLS. This option overrides _-_2_, _-_-_s_s_l_v_2 and _-_1_, _-_-_t_l_s_v_1 and _-_-_t_l_s_v_1_._1 and _-_-_t_l_s_v_1_._2.
       ...
       --suppress-connect-headers
              When _-_p_, _-_-_p_r_o_x_y_t_u_n_n_e_l is used and a CONNECT request is made do not output proxy CONNECT response headers. This option is meant to be used with _-_D_, _-_-_d_u_m_p_-_h_e_a_d_e_r or _-_i_, _-_-_i_n_c_l_u_d_e which  are
              used to show protocol headers in the output. It has no effect on debug options such as _-_v_, _-_-_v_e_r_b_o_s_e or _-_-_t_r_a_c_e, or any statistics.

              Example:
               curl --suppress-connect-headers --include -x proxy https://example.com

              See also _-_D_, _-_-_d_u_m_p_-_h_e_a_d_e_r, _-_i_, _-_-_i_n_c_l_u_d_e and _-_p_, _-_-_p_r_o_x_y_t_u_n_n_e_l. Added in 7.54.0.
       ...
       --tcp-fastopen
              ...
              See also _-_-_f_a_l_s_e_-_s_t_a_r_t. Added in 7.49.0.

       --tcp-nodelay
              Turn on the TCP_NODELAY option. See the _c_u_r_l___e_a_s_y___s_e_t_o_p_t_(_3_) man page for details about this option.
       ...
       --tftp-no-options
              ...
              This option improves interop with some legacy servers that do not acknowledge or properly implement TFTP options. When this option is used _-_-_t_f_t_p_-_b_l_k_s_i_z_e is ignored.

              Example:
               curl --tftp-no-options tftp://192.168.0.1/

              See also _-_-_t_f_t_p_-_b_l_k_s_i_z_e. Added in 7.48.0.
       ...
       -z, --time-cond <time>
              ...
              or if it does not match any internal ones, it is taken as a filename and tries to get the modification date (mtime) from <file> instead. See the _c_u_r_l___g_e_t_d_a_t_e_(_3_) man pages for  date  expres‐
              sion details.
       ...
       See also _-_-_t_l_s_v_1_._0, _-_-_t_l_s_v_1_._1, _-_-_t_l_s_v_1_._2 and _-_-_t_l_s_v_1_._3. _-_-_t_l_s_-_m_a_x requires that the underlying libcurl was built to support TLS. Added in 7.54.0.
       ...
       --tls13-ciphers <ciphersuite list>
              ...
              This option is currently used only when curl is built to use OpenSSL 1.1.1 or later. If you are using a different SSL backend you can try setting TLS 1.3 cipher suites by  using  the  _-_-_c_i_‐
              _p_h_e_r_s option.
              ...
              See also _-_-_c_i_p_h_e_r_s and _-_-_c_u_r_v_e_s. Added in 7.61.0.

       --tlsauthtype <type>
              Set  TLS  authentication type. Currently, the only supported option is "SRP", for TLS-SRP (RFC 5054). If _-_-_t_l_s_u_s_e_r and _-_-_t_l_s_p_a_s_s_w_o_r_d are specified but _-_-_t_l_s_a_u_t_h_t_y_p_e is not, then this option
              defaults to "SRP". This option works only if the underlying libcurl is built with TLS-SRP support, which requires OpenSSL or GnuTLS with TLS-SRP support.
       ...
       --tlspassword <string>
              Set password for use with the TLS authentication method specified with _-_-_t_l_s_a_u_t_h_t_y_p_e. Requires that _-_-_t_l_s_u_s_e_r also be set.
       ...
       --tlsuser <name>
              Set username for use with the TLS authentication method specified with _-_-_t_l_s_a_u_t_h_t_y_p_e. Requires that _-_-_t_l_s_p_a_s_s_w_o_r_d also is set.
       ...
       --tlsv1.0
              ...
              In old versions of curl this option was documented to allow _only_ TLS 1.0.  That behavior was inconsistent depending on the TLS library. Use _-_-_t_l_s_-_m_a_x if you want to set a maximum TLS ver‐
              sion.

              Example:
               curl --tlsv1.0 https://example.com

              See also _-_-_t_l_s_v_1_._3. Added in 7.34.0.

       --tlsv1.1
              (TLS) Forces curl to use TLS version 1.1 or later when connecting to a remote TLS server.

              In old versions of curl this option was documented to allow _only_ TLS 1.1.  That behavior was inconsistent depending on the TLS library. Use _-_-_t_l_s_-_m_a_x if you want to set a maximum TLS ver‐
              sion.
       ...
       --tlsv1.2
              ...
              In old versions of curl this option was documented to allow _only_ TLS 1.2.  That behavior was inconsistent depending on the TLS library. Use _-_-_t_l_s_-_m_a_x if you want to set a maximum TLS ver‐
              sion.
       ...
       --tlsv1.3
              ...
              See also _-_-_t_l_s_v_1_._2. Added in 7.52.0.
       ...
       -1, --tlsv1
              ...
              See also _-_-_h_t_t_p_1_._1 and _-_-_h_t_t_p_2. _-_1_, _-_-_t_l_s_v_1 requires that the underlying libcurl was built to support TLS. This option overrides _-_-_t_l_s_v_1_._1 and _-_-_t_l_s_v_1_._2 and _-_-_t_l_s_v_1_._3.
       ...
       --trace-ascii <file>
              ...
              This is similar to _-_-_t_r_a_c_e, but leaves out the hex part and only shows the ASCII part of the dump. It makes smaller output that might be easier to read for untrained humans.
              ...
              See also _-_v_, _-_-_v_e_r_b_o_s_e and _-_-_t_r_a_c_e. This option overrides _-_-_t_r_a_c_e and _-_v_, _-_-_v_e_r_b_o_s_e.
       ...
       --trace <file>
              ...
              See also _-_-_t_r_a_c_e_-_a_s_c_i_i and _-_-_t_r_a_c_e_-_t_i_m_e. This option overrides _-_v_, _-_-_v_e_r_b_o_s_e and _-_-_t_r_a_c_e_-_a_s_c_i_i.
       ...
       --unix-socket <path>
              ...
              See also _-_-_a_b_s_t_r_a_c_t_-_u_n_i_x_-_s_o_c_k_e_t. Added in 7.40.0.

       -T, --upload-file <file>
              This transfers the specified local file to the remote URL. If there is no file part in the specified URL, curl will append the local file name. NOTE that you must use a trailing  /  on  the
              last directory to really prove to Curl that there is no file name or curl will think that your last directory name is the remote file name to use. That will most likely cause the upload op‐
              eration to fail. If this is used on an HTTP(S) server, the PUT command will be used.

              Use the file name "-" (a single dash) to use stdin instead of a given file.  Alternately, the file name "." (a single period) may be specified instead of "-" to use  stdin  in  non-blocking
              mode to allow reading server output while stdin is being uploaded.

              You  can  specify  one  _-_T_, _-_-_u_p_l_o_a_d_-_f_i_l_e for each URL on the command line. Each _-_T_, _-_-_u_p_l_o_a_d_-_f_i_l_e + URL pair specifies what to upload and to where. curl also supports "globbing" of the _-_T_,
              _-_-_u_p_l_o_a_d_-_f_i_l_e argument, meaning that you can upload multiple files to a single URL by using the same URL globbing style supported in the URL.
              ...
              See also _-_G_, _-_-_g_e_t and _-_I_, _-_-_h_e_a_d.
       ...
       --url <url>
              ...
              POP3 or SMTP then that protocol will be used, otherwise HTTP will be used. Since 7.45.0 guessing can be disabled by setting a default protocol, see _-_-_p_r_o_t_o_-_d_e_f_a_u_l_t for details.

              This option may be used any number of times. To control where this URL is written, use the _-_o_, _-_-_o_u_t_p_u_t or the _-_O_, _-_-_r_e_m_o_t_e_-_n_a_m_e options.

              WWAARRNNIINNGG: On Windows, particular file:// accesses can be converted to network accesses by the operating system. Beware!
       ...
       -A, --user-agent <name>
              (HTTP) Specify the User-Agent string to send to the HTTP server. To encode blanks in the string, surround the string with single quote marks. This header  can  also  be  set  with  the  _-_H_,
              _-_-_h_e_a_d_e_r or the _-_-_p_r_o_x_y_-_h_e_a_d_e_r options.

              If you give an empty argument to _-_A_, _-_-_u_s_e_r_-_a_g_e_n_t (""), it will remove the header completely from the request. If you prefer a blank header, you can set it to a single space (" ").
       ...
       -u, --user <user:password>
              Specify the user name and password to use for server authentication. Overrides _-_n_, _-_-_n_e_t_r_c and _-_-_n_e_t_r_c_-_o_p_t_i_o_n_a_l.
              ...
              If  you  use a Windows SSPI-enabled curl binary and perform Kerberos V5, Negotiate, NTLM or Digest authentication then you can tell curl to select the user name and password from your envi‐
       ...
       -v, --verbose
              ...
              If you only want HTTP headers in the output, _-_i_, _-_-_i_n_c_l_u_d_e might be the option you are looking for.

              If you think this option still does not give you enough details, consider using _-_-_t_r_a_c_e or _-_-_t_r_a_c_e_-_a_s_c_i_i instead.

              This option is global and does not need to be specified for each use of _-_:_, _-_-_n_e_x_t.

              Use _-_s_, _-_-_s_i_l_e_n_t to make curl really quiet.

              Example:
               curl --verbose https://example.com

              See also _-_i_, _-_-_i_n_c_l_u_d_e. This option overrides _-_-_t_r_a_c_e and _-_-_t_r_a_c_e_-_a_s_c_i_i.
       ...
       -w, --write-out <format>
              ...
              NNOOTTEE:: The %-symbol is a special symbol in the win32-environment, where all occurrences of % must be doubled when using this option.

              The variables available are:

              ccoonntteenntt__ttyyppee   The Content-Type of the requested document, if there was any.

              eerrrroorrmmssgg       The error message. (Added in 7.75.0)

              eexxiittccooddee       The numerical exitcode of the transfer. (Added in 7.75.0)

              ffiilleennaammee__eeffffeeccttiivvee
                             The  ultimate filename that curl writes out to. This is only meaningful if curl is told to write to a file with the _-_O_, _-_-_r_e_m_o_t_e_-_n_a_m_e or _-_o_, _-_-_o_u_t_p_u_t option. It's most useful
                             in combination with the _-_J_, _-_-_r_e_m_o_t_e_-_h_e_a_d_e_r_-_n_a_m_e option.

              ffttpp__eennttrryy__ppaatthh The initial path curl ended up in when logging on to the remote FTP server.

              hhttttpp__ccooddee      The numerical response code that was found in the last retrieved HTTP(S) or FTP(s) transfer.

              hhttttpp__ccoonnnneecctt   The numerical code that was found in the last response (from a proxy) to a curl CONNECT request.

              hhttttpp__vveerrssiioonn   The http version that was effectively used. (Added in 7.50.0)

              jjssoonn           A JSON object with all available keys.

              llooccaall__iipp       The IP address of the local end of the most recently done connection - can be either IPv4 or IPv6.

              llooccaall__ppoorrtt     The local port number of the most recently done connection.

              mmeetthhoodd         The http method used in the most recent HTTP request. (Added in 7.72.0)

              nnuumm__ccoonnnneeccttss   Number of new connects made in the recent transfer.

              nnuumm__hheeaaddeerrss    The number of response headers in the most recent request (restarted at each
                              redirect). Note that the status line IS NOT a header. (Added in 7.73.0)

              nnuumm__rreeddiirreeccttss  Number of redirects that were followed in the request.

              oonneerrrroorr        The rest of the output is only shown if the transfer returned a non-zero error (Added in 7.75.0)

              pprrooxxyy__ssssll__vveerriiffyy__rreessuulltt
              ...
              rreeddiirreecctt__uurrll   When an HTTP request was made without _-_L_, _-_-_l_o_c_a_t_i_o_n to follow redirects (or when _-_-_m_a_x_-_r_e_d_i_r_s is met), this variable will show the actual URL a redirect _w_o_u_l_d have gone to.

              rreeffeerreerr        The Referer: header, if there was any. (Added in 7.76.0)

              rreemmoottee__iipp      The remote IP address of the most recently done connection - can be either IPv4 or IPv6.

              rreemmoottee__ppoorrtt    The remote port number of the most recently done connection.

              rreessppoonnssee__ccooddee  The numerical response code that was found in the last transfer (formerly known as "http_code").

              sscchheemmee         The URL scheme (sometimes called protocol) that was effectively used. (Added in 7.52.0)

              ssiizzee__ddoowwnnllooaadd  The total amount of bytes that were downloaded. This is the size of the body/data that was transferred, excluding headers.
              ...
              ssiizzee__uuppllooaadd    The total amount of bytes that were uploaded. This is the size of the body/data that was transferred, excluding headers.
              ...
              ssttddeerrrr         From this point on, the _-_w_, _-_-_w_r_i_t_e_-_o_u_t output will be written to standard error. (Added in 7.63.0)

              ssttddoouutt         From this point on, the _-_w_, _-_-_w_r_i_t_e_-_o_u_t output will be written to standard output.  This is the default, but can be used to switch back after switching to stderr.  (Added  in
                             7.63.0)
              ...
              ttiimmee__ccoonnnneecctt   The time, in seconds, it took from the start until the TCP connect to the remote host (or proxy) was completed.
              ...
              ttiimmee__rreeddiirreecctt  The time, in seconds, it took for all redirection steps including name lookup, connect, pretransfer and transfer before the final transaction was started. time_redirect shows
                             the complete execution time for multiple redirections.

              ttiimmee__ssttaarrttttrraannssffeerr
                             The time, in seconds, it took from the start until the first byte was just about to be transferred. This includes time_pretransfer and also the time the server needed to cal‐
                             culate the result.

              ttiimmee__ttoottaall     The total time, in seconds, that the full operation lasted.

              uurrll            The URL that was fetched. (Added in 7.75.0)

              uurrllnnuumm         The URL index number of this transfer, 0-indexed. De-globbed URLs share the same index number as the origin globbed URL. (Added in 7.75.0)

              uurrll__eeffffeeccttiivvee  The URL that was fetched last. This is most meaningful if you have told curl to follow location: headers.
...
EENNVVIIRROONNMMEENNTT
       ...
       Using an environment variable to set the proxy has the same effect as using the _-_x_, _-_-_p_r_o_x_y option.
       ...
       NO_PROXY <comma-separated list of hosts/domains>
              ...
              This environment variable disables use of the proxy even when specified with the _-_x_, _-_-_p_r_o_x_y option. That is NNOO__PPRROOXXYY==ddiirreecctt..eexxaammppllee..ccoomm ccuurrll --xx hhttttpp::////pprrooxxyy..eexxaammppllee..ccoomm hhttttpp::////ddiirreecctt..eexxaamm‐‐
              ppllee..ccoomm accesses the target URL directly, and NNOO__PPRROOXXYY==ddiirreecctt..eexxaammppllee..ccoomm ccuurrll --xx hhttttpp::////pprrooxxyy..eexxaammppllee..ccoomm hhttttpp::////ssoommeewwhheerree..eexxaammppllee..ccoomm accesses the target URL through the proxy.
       ...
       CURL_CA_BUNDLE <file>
              If set, will be used as the _-_-_c_a_c_e_r_t _v_a_l_u_e_.

       _C_U_R_L___H_O_M_E _<_d_i_r_>
              If set, is the first variable curl checks when trying to find its home directory. If not set, it continues to check XXDDGG__CCOONNFFIIGG__HHOOMMEE.
       ...
       HOME <dir>
              If set, this is used to find the home directory when that is needed. Like when looking for the default .curlrc. CCUURRLL__HHOOMMEE and XXDDGG__CCOONNFFIIGG__HHOOMMEE have preference.

       QLOGDIR <directory name>
              If curl was built with HTTP/3 support, setting this environment variable to a local directory will make curl produce qlogs in that directory, using file names named  after  the  destination
              connection id (in hex). Do note that these files can become rather large. Works with both QUIC backends.

       SHELL  Used on VMS when trying to detect if using a DCL or a "unix" shell.

       SSL_CERT_DIR <dir>
              If set, will be used as the _-_-_c_a_p_a_t_h _v_a_l_u_e_.

       _S_S_L___C_E_R_T___F_I_L_E _<_p_a_t_h_>
              If set, will be used as the _-_-_c_a_c_e_r_t _v_a_l_u_e_.

       _S_S_L_K_E_Y_L_O_G_F_I_L_E _<_f_i_l_e _n_a_m_e_>
       ...
       XDG_CONFIG_HOME <dir>
              If CCUURRLL__HHOOMMEE is not set, this variable is checked when looking for a default .curlrc file.
...
EEXXIITT CCOODDEESS
       ...
       22     HTTP page not retrieved. The requested url was not found or returned another error with the HTTP error code being 400 or above. This return code only appears if _-_f_, _-_-_f_a_i_l is used.
